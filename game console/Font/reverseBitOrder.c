...
#define NUM_CHARACTERS 255

// From http://www.piclist.com/techref/datafile/charset/8x6.htm accessed 22/08/2014
// Written by Philippe Lucidame
const char Font8x5[][CHARACTER_WIDTH] = {
		...
		{0x00,0x00,0x00,0x00,0x00,0x00},   //   0x20 32		<-- Start of visible characters
...
main(int argc, char **argv) {
		...
		for(i = 0; i <= NUM_CHARACTERS; i++) {
				for(j = 0; j < CHARACTER_WIDTH; j++) {
						tempC = reverse(Font8x5[i][j]);
						/* for(k = 7; k >= 0; k--) {
								tempC |= ((Font8x5[i][j] >> k) & 0x01) << i;								
						} */
						...
						printf("0x%02x", tempC & 0xFF);
						
						if(j == CHARACTER_WIDTH-1) {
								printf("}, //%c 0x%02x %3d \n",i, i, i);
						} else {
								printf(",");
						}
